...
SRC_URI="https://www.python.org/ftp/python/3.12.14/Python-3.12.14.tar.xz -> Python-3.12.14.tar.xz"
...
src_configure() {
	...
	if [[ -n "${disable}" ]];then
		echo "   *disabled*   " >> Modules/Setup.local || die
		for i in "${disable}"; do
			echo "   $i   " >> Modules/Setup.local || die
		done
	fi
